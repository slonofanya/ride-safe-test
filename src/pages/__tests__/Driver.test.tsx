...
const renderDriver = (initialFlags = {}) => {
  return render(
    <MemoryRouter>
...
describe("Driver Component", () => {
  ...
  it("should render loading state initially", () => {
    ...
    expect(screen.getByText("Loading driver rides...")).toBeInTheDocument();
  ...
  it("should render rides when data is loaded", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Driver Rides")).toBeInTheDocument();
    });
  ...
  it("should show map status as enabled when feature flag is on", async () => {
    ...
    renderDriver({ driverRouteMapPreview: true });

    await waitFor(() => {
      expect(screen.getByText("Map Preview: Enabled")).toBeInTheDocument();
    });
  });
  ...
  it("should show map status as disabled when feature flag is off", async () => {
    ...
    renderDriver({ driverRouteMapPreview: false });

    await waitFor(() => {
      expect(screen.getByText("Map Preview: Disabled")).toBeInTheDocument();
    });
  });
  ...
  it("should render empty state when no rides", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("No driver rides available")).toBeInTheDocument();
    });
  });
